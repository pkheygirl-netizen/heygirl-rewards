...
`;var x="/apps/loyalty";async function E(){try{let e=await fetch(`${x}/customer`);return e.ok?await e.json():null}catch{return null}}async function $(e=1,t=null){try{let o=new URLSearchParams({page:String(e)});t&&o.set("type",t);let r=await fetch(`${x}/history?${o}`);return r.ok?await r.json():{items:[],total:0}}catch{return{items:[],total:0}}}async function T(){try{let e=await fetch(`${x}/referral`);return e.ok?await e.json():null}catch{return null}}async function H(e){try{return await(await fetch(`${x}/redeem`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({requestedPoints:e})})).json()}catch{return{redeemed:!1,error:"network_error"}}}async function C(e){try{return(await fetch(`${x}/referral`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({referralSlug:e})})).ok}catch{return!1}}async function _(e){try{return await(await fetch(`${x}/social`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({platform:e})})).json()}catch{return{queued:!1}}}function p(e){return e.replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/"/g,"&quot;")}var M={silver:"\u25C8 Silver",gold:"\u2605 Gold",diamond:"\u25C6 Diamond"};function z(e){if(document.getElementById("hg-launcher"))return;let t=document.createElement("button");if(t.id="hg-launcher",t.className="hg-loading",!e.loggedIn||!e.member)t.classList.add("hg-gold"),t.innerHTML='<span class="hg-icon">\u{1F381}</span><span>Join &amp; Earn</span>',t.addEventListener("click",()=>{let o=window;typeof o.__hgTogglePreview=="function"?o.__hgTogglePreview():window.location.href="/account/register"});else{let{tier:o,balance:r}=e.member;t.className="",t.innerHTML=`
  ...
  <p class="hg-pv-signin">Already have an account? <a href="${ee}">Sign in</a></p>`;function re(){return te.map(e=>`<div class="hg-pv-row">
    ...
    </div>`,document.body.appendChild(t);let o=t.querySelector(".hg-pv-body"),r=t.querySelector("#hg-pv-back"),n=t.querySelector("#hg-pv-eyebrow"),d=t.querySelector("#hg-pv-title"),a={main:t.querySelector("#hg-pv-main"),earn:t.querySelector("#hg-pv-earn"),redeem:t.querySelector("#hg-pv-redeem")};function s(l){Object.values(a).forEach(h=>h.classList.remove("hg-active")),a[l].classList.add("hg-active");let i=l==="main";r.style.display=i?"none":"flex",n.style.display=i?"block":"none",d.classList.toggle("hg-pv-title-lg",i),o.scrollTop=0,i||r.focus()}function c(l){let i=document.getElementById("hg-launcher");i&&(l?(i.classList.add("hg-x"),i.innerHTML='<span class="hg-pv-x">\u2715</span>',i.setAttribute("aria-label","Close rewards preview")):(i.classList.remove("hg-x"),i.innerHTML='<span class="hg-icon">\u{1F381}</span><span>Join &amp; Earn</span>',i.setAttribute("aria-label","Open HeyGirl Rewards")))}function m(l){l.key==="Escape"&&b()}function g(){s("main"),t.classList.add("hg-open"),c(!0),document.addEventListener("keydown",m),t.querySelector("#hg-pv-close")?.focus()}function b(){t.classList.remove("hg-open"),c(!1),document.removeEventListener("keydown",m),document.getElementById("hg-launcher")?.focus()}function u(l,i){let h=t.querySelector(l);h?.addEventListener("click",()=>s(i)),h?.addEventListener("keydown",v=>{(v.key==="Enter"||v.key===" ")&&(v.preventDefault(),s(i))})}u("#hg-pv-go-earn","earn"),u("#hg-pv-go-redeem","redeem"),r.addEventListener("click",()=>s("main")),t.querySelector("#hg-pv-close")?.addEventListener("click",b);let f=window;f.__hgOpenPreview=g,f.__hgClosePreview=b,f.__hgTogglePreview=()=>t.classList.contains("hg-open")?b():g()}var N="hg_nudge1_shown",q="hg_nudge5_shown";function j(e,t){let o=e.nudgeSettings;if(o.nudge1_enabled&&!e.loggedIn&&!localStorage.getItem(N)&&setTimeout(()=>ie(),3e3),t==="cart"&&e.loggedIn&&e.member){let{balance:r,activeCodes:n}=e.member;o.nudge3_enabled&&n.length>0?ae(n[0]):o.nudge2_enabled&&r>=3e3&&se(r)}if(o.nudge5_enabled&&e.loggedIn&&e.member&&(t==="product"||t==="other")&&!sessionStorage.getItem(q)){let{member:r}=e;r.nextTier==="gold"&&r.spendToNextTier<=o.tier_progress_gold_threshold?A(`Only Rs.${r.spendToNextTier.toLocaleString()} away from Gold tier!`):r.nextTier==="diamond"&&r.spendToNextTier<=o.tier_progress_diamond_threshold&&A(`Only Rs.${r.spendToNextTier.toLocaleString()} away from Diamond tier!`)}}function ie(){let e=k("Earn points on every purchase","Sign up free and start earning Rs. rewards on every order at HeyGirl.pk.","Join & Earn","hg-nudge-br",()=>{window.location.href="/account/register"});document.body.appendChild(e),localStorage.setItem(N,"1")}function se(e){let t=B();if(!t)return;let o=k(`You have ${e.toLocaleString()} pts to spend`,"Redeem your points for a discount before checking out.","Redeem Now","hg-nudge-cart",()=>{let r=window.__hgOpenHub;typeof r=="function"&&r(0)});t.insertAdjacentElement("beforebegin",o)}function ae(e){let t=B();if(!t)return;let o=k(`You have an unused Rs.${e.discount_pkr} reward!`,`Apply code <strong>${p(e.code)}</strong> at checkout to save.`,"Copy Code","hg-nudge-cart",()=>{navigator.clipboard?.writeText(e.code)});t.insertAdjacentElement("beforebegin",o)}function A(e){let t=k("You're close to the next tier!",e,"View Tiers","hg-nudge-header",()=>{window.location.href="/pages/rewards"});document.body.prepend(t),sessionStorage.setItem(q,"1")}function B(){return document.querySelector(".cart__ctas")??document.querySelector('[name="checkout"]')?.closest("div")??document.querySelector(".cart")}function k(e,t,o,r,n){let d=document.createElement("div");return d.className=`hg-nudge hg-widget ${r}`,d.innerHTML=`
  ...
  `,d.querySelector(".hg-nudge-close")?.addEventListener("click",()=>d.remove()),d.querySelector(".hg-btn")?.addEventListener("click",()=>{n(),d.remove()}),d}function G(e){if(document.getElementById("hg-product-embed"))return;let t=window.ShopifyAnalytics?.meta?.product?.price??0,r=Math.floor(t/100),n=document.createElement("div");n.id="hg-product-embed",n.className="hg-widget",e.loggedIn?n.innerHTML=`<span>\u{1F381}</span> Earn <span class="hg-pts">${r.toLocaleString()} pts</span> on this purchase`:n.innerHTML=`<span>\u{1F381}</span> <a href="/account/login" style="color:#e91e8c;text-decoration:underline;">Sign in to earn ${r.toLocaleString()} pts</a> on this purchase`;let d=document.querySelector(".price__container")??document.querySelector(".product__info-wrapper .price")??document.querySelector("[data-product-price]");d&&d.insertAdjacentElement("afterend",n)}function D(e){if(document.getElementById("hg-cart-inline"))return;let t=document.createElement("div");if(t.id="hg-cart-inline",t.className="hg-widget",!e.loggedIn||!e.member)t.innerHTML='<span>\u{1F381}</span> <a href="/account/login" style="color:#e91e8c;">Sign in</a> to earn &amp; redeem rewards.';else{let{balance:r,activeCodes:n}=e.member,d=de(),a=d.some(c=>c.startsWith("REWARD-")),s=d.some(c=>!c.startsWith("REWARD-"));if(a)t.innerHTML="<span>\u2705</span> Your loyalty reward is applied. Enjoy the savings!";else if(s)t.innerHTML="<span>\u2139\uFE0F</span> A discount is already applied. Loyalty codes cannot stack.";else if(n.length>0){let c=n[0];t.innerHTML=`
    ...
    <div class="hg-rewards-hero">
      ...
      <div class="hg-progress-track">
        <div class="hg-progress-fill" style="width:${s}%;background:linear-gradient(90deg,#9c27b0,#e91e8c)"></div>
      </div>
  ...
  `,m=o.activeCodes.length>0?`<div class="hg-section">
      ...
      </div>`}).join(""),b=o.birthdayMonth&&o.birthdayMonth===r?`<div class="hg-birthday-card">
  ...
  `).join("")}function F(e,t){e.innerHTML=`
    <div class="hg-history-filter" id="hg-history-filter">
      ${me.map((d,a)=>`
        <button class="hg-filter-btn${a===0?" hg-active":""}" data-type="${d.key??""}">
          ${p(d.label)}
        </button>
      `).join("")}
    </div>
  ...
  `;let o=1,r=null;async function n(d,a){let s=e.querySelector("#hg-history-list"),c=e.querySelector("#hg-history-pagination");s.innerHTML=O(),c.innerHTML="";let{items:m,total:g}=await $(d,a);if(m.length===0){s.innerHTML='<p style="color:#888;font-size:13px;text-align:center;padding:32px 0;">No history yet \u2014 start earning points!</p>';return}s.innerHTML=m.map(u=>{let f=ge[u.action_type]??p(u.action_type),l=he[u.action_type]??"\u2B50",i=u.delta>=0,h=u.reason_note?`<div style="font-size:11px;color:#888;">${p(u.reason_note)}</div>`:"";return`
      ...
      `}).join("");let b=Math.ceil(g/50);b>1&&(c.innerHTML=`
        <button class="hg-page-btn" id="hg-hist-prev" ${d<=1?"disabled":""}>\u2190 Prev</button>
        <span style="font-size:13px;padding:6px 8px;">Page ${d} of ${b}</span>
        <button class="hg-page-btn" id="hg-hist-next" ${d>=b?"disabled":""}>Next \u2192</button>
      `,c.querySelector("#hg-hist-prev")?.addEventListener("click",()=>{o--,n(o,r)}),c.querySelector("#hg-hist-next")?.addEventListener("click",()=>{o++,n(o,r)}))}e.querySelector("#hg-history-filter")?.querySelectorAll(".hg-filter-btn").forEach(d=>{d.addEventListener("click",()=>{e.querySelectorAll(".hg-filter-btn").forEach(s=>s.classList.remove("hg-active")),d.classList.add("hg-active");let a=d.dataset.type??"";r=a===""?null:a,o=1,n(1,r)})}),n(1,null)}var fe=[{platform:"youtube",label:"YouTube",pts:1e3,icon:"\u25B6"},{platform:"facebook",label:"Facebook",pts:1e3,icon:"f"},{platform:"instagram",label:"Instagram",pts:1e3,icon:"\u{1F4F7}"}];async function W(e,t){e.innerHTML='<p style="color:#888;font-size:13px;text-align:center;padding:20px 0;">Loading\u2026</p>';let o=null;try{o=await T()}catch{e.innerHTML='<p class="hg-empty-state">Could not load referral data. Please try again later.</p>';return}if(!o){e.innerHTML='<p style="color:#888;font-size:13px;text-align:center;padding:20px 0;">Referral data unavailable.</p>';return}let{referralLink:r,totalReferrals:n,completedReferrals:d,totalPtsEarned:a,history:s,isInfluencer:c,referralRate:m}=o,g=p(r),b=encodeURIComponent(`Join HeyGirl.pk and earn 1,000 welcome points! Use my link: ${r}`),u=c?`<div class="hg-influencer-badge">
      ...
      </div>`:"",f=s.length>0?`<div class="hg-section-title">Referral History</div>
         ${s.slice(0,10).map(i=>`<div class="hg-history-item">
               <div>
    ...
    <div class="hg-social-list" id="hg-social-list">
      ${fe.map(i=>`<div class="hg-social-row">
  ...
  `,e.querySelector("#hg-ref-copy")?.addEventListener("click",i=>{let h=i.currentTarget;navigator.clipboard?.writeText(r).then(()=>{h.textContent="Copied!",setTimeout(()=>{h.textContent="Copy Link"},2e3)})});let l=e.querySelector("#hg-social-msg");e.querySelectorAll("[data-platform]").forEach(i=>{i.addEventListener("click",async()=>{let h=i.dataset.platform,v=i.textContent;i.textContent="Submitting\u2026",i.disabled=!0;let y=await _(h);y.alreadyClaimed?(l.textContent="Already claimed for this platform.",l.className="hg-social-msg hg-msg-warn",i.textContent="Claimed \u2713"):y.queued?(l.textContent="Submitted! Points pending 24h review",l.className="hg-social-msg hg-msg-ok",i.textContent="Queued \u2713"):(l.textContent="Something went wrong. Please try again.",l.className="hg-social-msg hg-msg-err",i.textContent=v??"I Followed!",i.disabled=!1)})})}var be=[{key:"silver",name:"\u25C8 Silver",color:"#9e9e9e",threshold:"Default",benefits:["Earn 1 pt per Rs.1 spent","Sign-up bonus: 1,000 pts","Birthday reward: Rs.250","Referral bonus: 6,000 pts","Points expire after 1 year (FIFO)"]},{key:"gold",name:"\u2605 Gold",color:"#f9a825",threshold:"Rs.50,000 lifetime spend",benefits:["Earn 1 pt per Rs.1 spent","Points never expire","Birthday reward: Rs.500","Referral bonus: 6,000 pts","Priority customer support"]},{key:"diamond",name:"\u25C6 Diamond",color:"#7b1fa2",threshold:"Rs.100,000 lifetime spend",benefits:["Earn 1.2\xD7 points in birthday month","Points never expire","Birthday reward: Rs.1,000","Referral bonus: 6,000 pts","Early access to new arrivals","Exclusive Reward Room access"]}];function J(e,t){if(!t.member){e.innerHTML="<p style='color:#888;text-align:center;padding:16px;'>Please log in to view tier info.</p>";return}let{tier:o,lifetimeSpend:r,spendToNextTier:n,nextTier:d}=t.member,a=t.nudgeSettings?.tier_progress_gold_threshold??5e4,s=t.nudgeSettings?.tier_progress_diamond_threshold??1e5,c=Math.min(100,Math.round(r/a*100)),m=Math.min(100,Math.round(r/s*100));e.innerHTML=`
    ...
    <div class="hg-tier-progress-section">
      ...
      <div style="font-size:13px;color:#555;margin-bottom:12px;">
        ...
        ${d?` \xB7 Rs.${n.toLocaleString()} more to reach ${d.charAt(0).toUpperCase()+d.slice(1)}`:" \xB7 Maximum tier reached!"}
      </div>
      <div class="hg-progress-label">
        <span>Silver \u2192 Gold (Rs.${a.toLocaleString()})</span><span>${c}%</span>
      </div>
      <div class="hg-progress-track">
        <div class="hg-progress-fill" style="width:${c}%;background:#f9a825;"></div>
      </div>
      <div class="hg-progress-label" style="margin-top:8px;">
        <span>Gold \u2192 Diamond (Rs.${s.toLocaleString()})</span><span>${m}%</span>
      </div>
      <div class="hg-progress-track">
