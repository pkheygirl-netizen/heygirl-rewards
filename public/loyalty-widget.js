...
`;var k="/apps/loyalty";async function p(){try{let n=await fetch(`${k}/customer`);return n.ok?await n.json():null}catch{return null}}var g={silver:"\u25C8 Silver",gold:"\u2605 Gold",diamond:"\u25C6 Diamond"};function u(n){if(document.getElementById("hg-launcher"))return;let e=document.createElement("button");if(e.id="hg-launcher",e.className="hg-loading",!n.loggedIn||!n.member)e.innerHTML='<span class="hg-icon">\u{1F381}</span><span>Join &amp; Earn</span>',e.addEventListener("click",()=>{window.location.href="/account/register"});else{let{tier:t,balance:r}=n.member;e.className="",e.innerHTML=`
  ...
  `,document.body.appendChild(t),t.querySelector("#hg-hub-btn")?.addEventListener("click",()=>{let s=window.__hgOpenHub;typeof s=="function"?s():window.location.href="/pages/rewards",t.classList.remove("hg-open")}),t.querySelector("#hg-redeem-btn")?.addEventListener("click",()=>{let s=window.__hgOpenHub;typeof s=="function"&&s(0),t.classList.remove("hg-open")}),t.querySelector("#hg-copy-ref")?.addEventListener("click",s=>{let d=s.currentTarget;navigator.clipboard?.writeText(`https://${i}`).then(()=>{d.textContent="Copied!",setTimeout(()=>{d.textContent="Copy"},2e3)})}),document.addEventListener("click",s=>{let d=document.getElementById("hg-launcher");!t.contains(s.target)&&s.target!==d&&t.classList.remove("hg-open")})}var f="hg_nudge1_shown",b="hg_nudge5_shown";function x(n,e){let t=n.nudgeSettings;if(t.nudge1_enabled&&!n.loggedIn&&!localStorage.getItem(f)&&setTimeout(()=>_(),3e3),e==="cart"&&n.loggedIn&&n.member){let{balance:r,activeCodes:o}=n.member;t.nudge3_enabled&&o.length>0?C(o[0]):t.nudge2_enabled&&r>=3e3&&E(r)}if(t.nudge5_enabled&&n.loggedIn&&n.member&&(e==="product"||e==="other")&&!sessionStorage.getItem(b)){let{member:r}=n;r.nextTier==="gold"&&r.spendToNextTier<=t.tier_progress_gold_threshold?m(`Only Rs.${r.spendToNextTier.toLocaleString()} away from Gold tier!`):r.nextTier==="diamond"&&r.spendToNextTier<=t.tier_progress_diamond_threshold&&m(`Only Rs.${r.spendToNextTier.toLocaleString()} away from Diamond tier!`)}}function _(){let n=c("Earn points on every purchase","Sign up free and start earning Rs. rewards on every order at HeyGirl.pk.","Join & Earn","hg-nudge-br",()=>{window.location.href="/account/register"});document.body.appendChild(n),localStorage.setItem(f,"1")}function E(n){let e=y();if(!e)return;let t=c(`You have ${n.toLocaleString()} pts to spend`,"Redeem your points for a discount before checking out.","Redeem Now","hg-nudge-cart",()=>{let r=window.__hgOpenHub;typeof r=="function"&&r(0)});e.insertAdjacentElement("beforebegin",t)}function C(n){let e=y();if(!e)return;let t=c(`You have an unused Rs.${n.discount_pkr} reward!`,`Apply code <strong>${n.code}</strong> at checkout to save.`,"Copy Code","hg-nudge-cart",()=>{navigator.clipboard?.writeText(n.code)});e.insertAdjacentElement("beforebegin",t)}function m(n){let e=c("You're close to the next tier!",n,"View Tiers","hg-nudge-header",()=>{window.location.href="/pages/rewards"});document.body.prepend(e),sessionStorage.setItem(b,"1")}function y(){return document.querySelector(".cart__ctas")??document.querySelector('[name="checkout"]')?.closest("div")??document.querySelector(".cart")}function c(n,e,t,r,o){let i=document.createElement("div");return i.className=`hg-nudge hg-widget ${r}`,i.innerHTML=`
  ...
  `,i.querySelector(".hg-nudge-close")?.addEventListener("click",()=>i.remove()),i.querySelector(".hg-btn")?.addEventListener("click",()=>{o(),i.remove()}),i}function w(n){if(document.getElementById("hg-product-embed"))return;let e=window.ShopifyAnalytics?.meta?.product?.price??0,r=Math.floor(e/100),o=document.createElement("div");o.id="hg-product-embed",o.className="hg-widget",n.loggedIn?o.innerHTML=`<span>\u{1F381}</span> Earn <span class="hg-pts">${r.toLocaleString()} pts</span> on this purchase`:o.innerHTML=`<span>\u{1F381}</span> <a href="/account/login" style="color:#e91e8c;text-decoration:underline;">Sign in to earn ${r.toLocaleString()} pts</a> on this purchase`;let i=document.querySelector(".price__container")??document.querySelector(".product__info-wrapper .price")??document.querySelector("[data-product-price]");i&&i.insertAdjacentElement("afterend",o)}function v(n){if(document.getElementById("hg-cart-inline"))return;let e=document.createElement("div");if(e.id="hg-cart-inline",e.className="hg-widget",!n.loggedIn||!n.member)e.innerHTML='<span>\u{1F381}</span> <a href="/account/login" style="color:#e91e8c;">Sign in</a> to earn &amp; redeem rewards.';else{let{balance:r,activeCodes:o}=n.member,i=L(),s=i.some(a=>a.startsWith("REWARD-")),d=i.some(a=>!a.startsWith("REWARD-"));if(s)e.innerHTML="<span>\u2705</span> Your loyalty reward is applied. Enjoy the savings!";else if(d)e.innerHTML="<span>\u2139\uFE0F</span> A discount is already applied. Loyalty codes cannot stack.";else if(o.length>0){let a=o[0];e.innerHTML=`
      ...
      `,setTimeout(()=>{document.getElementById("hg-open-redeem")?.addEventListener("click",()=>{let a=window.__hgOpenHub;typeof a=="function"&&a(0)})},0);else{let a=3e3-r;e.innerHTML=`<span>\u{1F381}</span> Earn <strong>${a.toLocaleString()} more pts</strong> to unlock your first reward.`}}let t=document.querySelector('[name="checkout"]')??document.querySelector(".cart__checkout");t&&t.insertAdjacentElement("beforebegin",e)}function L(){let n=window.Shopify?.checkout;return n?.discount?.applicable&&n.discount.code?[n.discount.code]:[]}(function(){"use strict";function n(){if(document.getElementById("hg-styles"))return;let o=document.createElement("style");o.id="hg-styles",o.textContent=l,document.head.appendChild(o)}function e(){let o=window.location.pathname;return o==="/pages/rewards"?"rewards":o.startsWith("/products/")?"product":o==="/cart"?"cart":"other"}async function t(){try{n();let o=e(),i=await p();if(!i)return;o!=="rewards"&&(u(i),h(i)),i.nudgeSettings&&x(i,o),o==="product"&&w(i),o==="cart"&&v(i),o==="rewards"&&void 0}catch{}}function r(o){}window.__hgOpenHub=null,document.readyState==="loading"?document.addEventListener("DOMContentLoaded",t):t()})();})();
